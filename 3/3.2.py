...
class Encoder(nn.Module):
    ...
    def __init__(self, in_chan, hidden_ch, out_channels):
        ...
        self.conv1 = nn.Conv2d(in_chan, hidden_ch, kernel_size=3, stride=1, padding=1) # 28 x28
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_chan, hidden_ch, out_chan):
        ...
        self.conv3 = nn.Conv2d(hidden_ch, out_chan, kernel_size=3, stride=1, padding=1)
...
#dataset
dataset = datasets.MNIST('/adg', download=True)
...
#
test = dataset.data[5].unsqueeze(0).unsqueeze(0).float() / 255
predict = model(test)
